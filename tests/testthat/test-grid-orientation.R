...
test_that("piepho components match hand-derived values on a non-square grid", {
  ...
  # row 1:  a  c  b  a  c  b
  # row 2:  b  a  c  b  a  c
  # Six columns to two rows, so neighbour balance is counted along rows only
  # (Piepho et al. 2021). Row 1 contributes (a,c) (b,c) (a,b) (a,c) (b,c) and
  # row 2 contributes (a,b) (a,c) (b,c) (a,b) (a,c), giving a,b = 3, a,c = 4,
  # b,c = 3.
  df <- initialise_design_df(
    items = rep(c("a", "b", "c"), 4),
    nrows = 2,
  ...
  counts <- c(3, 4, 3)
  expect_equal(
    as.integer(res$nb$nb[c("a,b", "a,c", "b,c")]),
    as.integer(counts)
  )
  expect_equal(res$components[["neighbour_balance"]], var(counts))
  expect_equal(res$nb$s2, sum(counts * (counts - 1) / 2))

  # No like-treatment neighbours anywhere in this layout, in either direction.
  expect_equal(res$components[["self_adjacency"]], 0)
  expect_equal(calculate_adjacency_score(df, "treatment"), 0)

  # Evenness is the sum of 1/MST_i over the replicated treatments.
  expect_equal(
    res$components[["even_distribution"]],
    res$ed[["1"]]$inv_total_mst
  )
  expect_equal(sum(res$components), res$score)
})

test_that("piepho does not overwrite the treatment column it was given", {
  # The metrics must come out identical to computing them on `design` directly,
  # i.e. the treatments stay aligned with their coordinates.
  df <- initialise_design_df(
  ...
  dm <- build_design_matrix(df, "treatment")
  expect_equal(res$nb$nb, calculate_nb(dm, pm)$nb)
  expect_equal(res$nb$self_adjacencies, calculate_nb(dm, pm)$self_adjacencies)
  expect_equal(res$ed[["1"]]$msts, calculate_ed(dm)$msts)
})
...
test_that("neighbour balance skips empty cells, with and without a mapping", {
  ...
  # (rows 4-5). Nothing crosses the gap. Both directions are asked for
  # explicitly: this layout has more rows than columns, so the default would
  # count down columns only.
  df <- data.frame(
  ...
  with_mapping <- calculate_nb(
    m,
    create_pair_mapping(df$trt),
    directions = dirs
  )
  without_mapping <- calculate_nb(m, directions = dirs)

  # The two C-C adjacencies are self-pairs, reported apart from distinct pairs.
  expect_equal(with_mapping$self_adjacencies, 2L)
  expect_equal(without_mapping$self_adjacencies, 2L)
  expect_equal(
    as.integer(with_mapping$nb[c("A,B", "A,C", "B,C")]),
    c(2L, 2L, 2L)
  )
