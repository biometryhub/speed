test_that("initialize_design_df works for simple design", {
  ...
  expect_equal(design_df$treatment, treatments)
...
test_that("initialize_design_df works for blocked design", {
  ...
  expect_equal(sort(design_df$row), sort(rep(1:nrows, ncols)))
  expect_equal(sort(design_df$col), sort(rep(1:ncols, nrows)))
  expect_equal(sort(design_df$row_block), sort(rep(1:(nrows / nrows_block), ncols * nrows_block)))
  expect_equal(sort(design_df$col_block), sort(rep(1:(ncols / ncols_block), nrows * ncols_block)))
  expect_equal(design_df$treatment, treatments)
  expect_equal(sum(design_df[is.null(design_df), ]), 0)
})
