...
test_that("adjacency_score_vec scores per-cell matches", {
  ...
  expect_equal(
    score_uniform,
    matrix(c(2L, 3L, 2L, 3L, 4L, 3L, 2L, 3L, 2L), nrow = 3, byrow = TRUE)
  )
})

test_that("adjacency_score_vec applies per-ring weights", {
  # fmt: skip
  m <- matrix(c(1L, 2L, 1L, 3L,
                2L, 1L, 2L, 1L,
                1L, 3L, 1L, 2L,
                2L, 1L, 2L, 1L),
              nrow = 4, byrow = TRUE)
  ...
  expect_true(any(ring_2_only > 0L))
  # ring 2 contribution is scaled by (10 - 1) = 9 per matching neighbour
  expect_true(all(ring_2_only %% 9L == 0L))
})
