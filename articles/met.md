...
Now we can create a data frame representing a MET design. Note that the
`initalise_design_df` function does not currently support split plot
designs directly, so we will create it manually.
