...
While simple to implement, CRD doesn’t account for spatial variation in
field conditions.
...
each. Even though treatments are assigned randomly, spatial optimization
can reduce neighbor effects and improve precision.
...
effects. Firstly we will initialise a dataframe representing the design
...
![](common_designs_files/figure-html/crd-plot1-1.png)
...
case, we only need to provide the design dataframe for it to operate on,
the column name of the dataframe to use for the treatments, and a seed
for reproducibility.
...
```
    ...
    Iteration: 1000 Score: 2.285714 Best: 2.285714 Since Improvement: 271
...
![](common_designs_files/figure-html/crd-plot2-1.png)
...
Consider testing 6 soybean varieties across a field with a moisture
gradient. Using 4 blocks perpendicular to the gradient helps control for
soil moisture variation.

### Setting Up RCBD with speed

Here we initialise a dataframe for a design with 4 blocks and 6
...
![](common_designs_files/figure-html/rcbd-plot1-1.png)
...
```
    ...
    Iteration: 1000 Score: 1.6 Best: 1.6 Since Improvement: 672
...
![](common_designs_files/figure-html/rcbd-plot2-1.png)
...
![](common_designs_files/figure-html/latin-square-plot1-1.png)
...
```
    ...
    Iteration: 1000 Score: 1 Best: 1 Since Improvement: 308
...
![](common_designs_files/figure-html/latin-square-plot2-1.png)
...
Now we can create a dataframe representing a split plot design. Note
...
![](common_designs_files/figure-html/split-plot-plot1-1.png)

![](common_designs_files/figure-html/split-plot-plot1-2.png)

This is a systematic split plot layout; each treatment appears once per
block for wholeplot treatments, and once per whole plot for sub-plot
treatments. The design will now be randomised.

#### Performing the Optimisation

For split-plot designs, we use named lists to specify the hierarchical
structure. The `swap` parameter defines what to optimize at each level,
...
The output shows optimization results for the design. The score and
...
to assess the quality of optimization at each hierarchy level.
...
![](common_designs_files/figure-html/split-plot-plot2-1.png)
...
![](common_designs_files/figure-html/split-plot-plot2-2.png)
...
- **Long, narrow fields**: Favor designs with blocks perpendicular to
  the long axis
