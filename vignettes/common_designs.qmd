...
This vignette demonstrates how to use `speed` for common agricultural experimental designs, showing how spatial optimisation can improve the efficiency and validity of field trials.
...
Here we initialise a dataframe for a design with 4 blocks and 6 treatments, arranged in 4 rows and 6 columns.

```{r rcbd-df}
rcbd_design <- initialise_design_df(items = 6, nrows = 4, ncols = 6, block_nrows = 1, block_ncols = 6)
head(rcbd_design)
```
...
Split-Plot Design is used when some treatments are easier to apply to large areas (whole plots) while others require smaller areas (sub-plots). This creates a hierarchical structure with different levels of precision.

## When to Use

- Treatments with different application scales
...
## Setting Up Split-Plot with speed

```{r split-plot-example, eval=FALSE}
# Placeholder for Split-Plot example
# This will show:
# - Defining whole plot and sub-plot factors
# - Arranging whole plots optimally
# - Randomising sub-plots within whole plots
# - Using speed for spatial optimisation
# - Visualising the hierarchical structure
# - Understanding different error terms
```
...
Measures the average variance of treatment contrasts relative to an optimal design.

## D-efficiency

Based on the determinant of the information matrix, relating to confidence ellipsoid volume.

## E-efficiency

Focuses on the worst-case treatment contrast, ensuring balanced precision.

## Practical Considerations

```{r efficiency-comparison, eval=FALSE}
# Placeholder for efficiency comparison
# This will show:
# - Calculating efficiency metrics for different designs
# - Comparing optimised vs non-optimised layouts
# - Trade-offs between different efficiency measures
# - Guidelines for acceptable efficiency levels
```
...
Whether using simple randomised designs or complex incomplete block structures, spatial optimisation through `speed` can significantly enhance the efficiency of agricultural experiments.
