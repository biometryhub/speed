...
Each level of a hierarchical design swaps its own column, so companion columns have to be matched to the level that moves them. Pass a named list whose names match the levels:
...
linked_split <- speed(
  split_plot_linked,
  optimise = optimise,
  linked_cols = list(wp = "irrigation"),
  seed = 42
)
...
::: {.callout-note}
A bare character vector applies to every level, which in a hierarchical design is only valid when all levels swap the same column - a multi-environment trial, for example. Where levels swap different columns, use the named list, since a column can only travel with one `swap` column.
:::

Columns already used as `swap`, `swap_within`, or as spatial factors cannot be linked, as they are part of the optimisation rather than passengers on it.
