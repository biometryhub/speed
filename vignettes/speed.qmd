...
Each level of a hierarchical design swaps its own column, so companion columns have to be matched to the level that moves them. Since the hierarchy is already described level by level through `optimise`, `linked_cols` goes in alongside the other per-level arguments. Here the whole plot treatment carries an irrigation method, and the sub-plot treatment carries a cultivar name:
...
The same thing can be said from outside `optimise`, with a named list whose names match the levels - useful when the `optimise` list is defined elsewhere and reused:
...
Where a level sets `linked_cols` itself, that wins; the argument fills in the levels that do not.
