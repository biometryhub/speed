...
The printed output from the returned design object is a compact identity card: the field layout, the treatments and their replication, the final optimisation score (lower is better), how many iterations were run (and whether it stopped early), and the seed. For a fuller, statistically meaningful evaluation of the design, use `summary()` (see [Evaluating a Design](#evaluating-a-design)). The output object contains some additional components, which can be seen below:
...
- **Optimisation** -- the objective used, the final score decomposed into its **adjacency** and **balance** components (each shown as `raw x weight = contribution`), the initial-to-final score, the iterations run and the temperature schedule.
- **Evaluation** -- design-quality diagnostics:
  - **Connectedness** -- whether treatment effects are estimable. For a row--column design this fits `lm(~ row + col + treatment)` and checks for aliased terms; designs with a block factor fall back to a block-connectivity check.
...
Now we can create a data frame representing a split plot design. We can build it with `initialise_design_df()` by passing a `splits` list - one entry per nested level, each describing the size of one unit and the treatments to allocate to it.
