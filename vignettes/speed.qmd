...
ggplot2::ggplot(met_design, aes(col, row, fill = block)) +
...
For MET designs, we use named lists to specify the hierarchical structure. The `swap` parameter defines what to optimise at each level, while `swap_within` defines the constraints for each level.
