...
Building on the foundational designs covered in the [Common Agricultural Experimental Designs vignette](/common_designs.html), these complex designs offer solutions for challenging experimental scenarios while maintaining statistical rigour through spatial optimisation with the `speed` package.
...
Split-split plot designs extend the hierarchical structure of [split-plot designs](/common_designs.html#split-plot-design) to accommodate three factors with different levels of experimental difficulty or practical constraints. This creates a three-level hierarchy of experimental units.
...
result_ss <- speed(split_split_df,
                ...
                swap_within = list(wp = "block",
                                   sp = "wholeplot",
                                   ssp = "subplot"),
                iterations = list(wp = 2000, sp = 5000, ssp = 20000),
                early_stop_iterations = list(wp = 1000, sp = 3000, ssp = 5000),
                seed = 42)
...
```{r resolvable-incomplete-block-df}
...
ribd_df <- data.frame(
  ...
  treatment = rep(1:6, times = 3)                # Treatment assignments
)
...
#| echo: false
#| label: fig-ribdplot2
#| fig-cap: "Initial layout showing blocks."
autoplot(ribd_df, treatments = "block")
```

```{r}
#| echo: false
#| label: fig-ribdplot3
#| fig-cap: "Initial layout showing replicates."
autoplot(ribd_df, treatments = "replicate")
```

@fig-ribdplot1, @fig-ribdplot2 and @fig-ribdplot3 show the systematic resolvable incomplete block layout before optimisation. Note that replicates are made of two blocks, in each of which half the treatments will appear, and replicates are made up of pairs of blocks. The treatments can be swapped between blocks, but need to remain within their replicate.
