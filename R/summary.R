...
#' [create_pair_mapping()]/[calculate_nb()]. Both directions are always counted:
#' this describes the layout, rather than applying the orientation rule
#' `calculate_nb()` uses for the Piepho criterion.
#'
...
.neighbour_balance <- function(df, swap, rc, cc, grid) {
  ...
  # calculate_nb() already reports every distinct pair, self-pairs excluded and
  # counted separately, so only the distinct-pair counts accumulate here.
  all_pairs <- attr(pair_mapping, "pairs")
  ...
  for (g in grid) {
    ...
    nb <- calculate_nb(dm, pair_mapping, directions = c("row", "col"))
    counts[names(nb$nb)] <- counts[names(nb$nb)] + nb$nb
    self_adjacent <- self_adjacent + nb$self_adjacencies
  }

  return(list(
    available = TRUE,
    self_adjacent = self_adjacent,
    n_pairs = length(counts),
    min_pair_count = min(counts),
    max_pair_count = max(counts),
    pair_var = stats::var(counts),
    n_zero_pairs = sum(counts == 0)
  ))
}
