...
speed_matrix <- function(
    ...
    obj_function = objective_function_matrix(),
    quiet = FALSE,
    seed = NULL) {
  ...
  for (iter in 1:iterations) {
    ...
    new_design <- generate_neighbor_matrix(current_design, swap_matrix, current_swap_count, current_swap_all_blocks)
  ...
  output <- list(
    ...
    design_df = design_df,
    score = best_score,
    # adjacency_score = calculate_adjacency_score_matrix(best_design),
