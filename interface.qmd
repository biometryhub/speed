...
2. Multi factor designs _with_ hierarchy (e.g. split-plot, strip-plot, MET). These are more complex, and require a more flexible interface to specify the different levels of nesting and the relationships between them. These designs are partially supported in the current interface.
3. Multi factor designs _without_ hierarchy (e.g. factorial designs with multiple factors that are crossed). These are also complex, and require a different approach to specifying the design. These designs are not currently supported in the package.
...
For nested list interface, a new argument `optimise` is provided to accept a (named) list of named lists of speed arguments, so the optimisation algorithm can optimise the design with different arguments in order.
...
rcb_design <- speed(df_rcb,
                    swap = "treatment",
                    swap_within = "block",
                    # spatial_factors can be omitted to use its default value of ~ row + col
                    spatial_factors = ~ row + col)
```
...
sp_design <- speed(df_split,
                   ...
                   swap_within = list(wp = "block",
                                      sp = "wholeplot"),
                   # same as spatial_factors = ~ row + col
...
# again spatial_factors can always be omitted to use its default value
wp <- list(swap = all("wp_treatment"), swap_within = "block", spatial_factors = ~ row + col)
sp <- list(swap = "sp_treatment", swap_within = "wholeplot", spatial_factors = ~ row + col)
...
# or alternatively
fac_design <- speed(df_fac, optimise = list(swap = ~A*B,
                                            swap_within = ~block,
                                            spatial_factors = ~ row + col))
```
